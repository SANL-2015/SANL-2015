...
void test_lock() {
	...
	for (i = 0; i < nb_threads; i++) {
		liblock_thread_create_and_bind(
				&topology->cores[(i % (topology->nb_cores - 1))], 0, &tid[i], 0,
				thread_routine, (void*) (uintptr_t) i);
		n = (n + 1) % nb_threads;
	}
...
void* bad_synchro(void* _arg) {
	...
	case 0:
		...
		liblock_lock_init(liblock_name,
				&topology->cores[topology->nb_cores - 1], &other_lock, 0);
...
void two_locks() {
	...
	liblock_lock_init(liblock_name, &topology->cores[topology->nb_cores - 1],
			&lock2, 0);
...
int main(int argc, char** argv) {
	...
	liblock_reserve_core_for(&topology->cores[topology->nb_cores - 1],
			liblock_name);
	liblock_lock_init(liblock_name, &topology->cores[topology->nb_cores - 1],
			&lock, 0);
