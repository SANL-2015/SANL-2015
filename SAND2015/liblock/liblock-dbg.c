...
char log_buffer[MAX_NUM_IDS][65536];
uint64_t selves[MAX_NUM_IDS];
...
#define GET_NODES_CMD																										\
	...
	"if [ -d $NODES ]; then"																							\
  "  for F in `ls -d $NODES/node*`; do if [ $(cat $F/cpulist | grep '-') ]; then seq -s ' ' $(cat $F/cpulist | tr -s '-' ' '); else cat $F/cpulist | tr -s ',' ' '; fi; done;" \
	"else"																																\
...
__thread struct thread_descriptor self = { 0, 0, 0, {0,0}};
...
void liblock_reserve_core_for(struct core* core, const char* server_type) {
	if(__sync_val_compare_and_swap(&core->server_type, 0, server_type) != 0) {
		...
		//fprintf(stderr, "Binding a server %s to core %d\n", server_type, core->core_id);
	}
}
...
static void* my_start_routine(void* arg) {
	...
	struct liblock_info*  cur;
	void* res;
    //fprintf(stderr, "pre +++ starting routine: %d with core %d - %d\n", self.id, self.running_core ? self.running_core->core_id : -1, sched_getcpu());
    //^hangs, 1 core = 100%

//	fprintf(stderr, "--- starting routine: %d with core %d - %d %p\n", self.id, self.running_core ? self.running_core->core_id : -1, sched_getcpu(), r->start_routine);
//    fprintf(stderr, "pre +++ 1 starting routine\n");

	//if (sched_getcpu() == 4) fprintf(stderr, "--- starting routine: %d with core %d - %d %p\n", self.id, self.running_core ? self.running_core->core_id : -1, 0, r->start_routine);

	self.id = liblock_find_id(&id_manager);
    selves[self.id] = pthread_self();
    //log_print("got id\n");

//	fprintf(stderr, "--- starting routine: %d with core %d - %d %p\n", self.id, self.running_core ? self.running_core->core_id : -1, sched_getcpu(), r->start_routine);
	liblock_define_core(r->core);

//    fprintf(stderr, "pre +++ 2 starting routine\n");
	
    if(!r->core)
		liblock_auto_bind();

//    fprintf(stderr, "pre +++ 3 starting routine\n");
	...
	//fprintf(stderr, "--- starting routine: %d with core %d - %d %p\n", self.id, self.running_core ? self.running_core->core_id : -1, sched_getcpu(), r->start_routine);
//    fprintf(stderr, "pre +++ 4 starting routine\n");
...
int liblock_thread_create_and_bind(struct core* core, const char* server_type, pthread_t *thread, 
    ...
    //rclprintf(server, "works?");
    //fprintf(stderr, "build thread on %d\n", core ? core->core_id : -1);
    // ^ Rarely crashes if here. EDIT: false, actually.    

	struct start_routine* r = liblock_allocate(sizeof(struct start_routine));
	//struct start_routine* r = anon_mmap(sizeof(struct start_routine));
    //*((int *)r)=0; < still crashes with this, anon_mmap also makes it crash
	/*if (!strcmp(server_type, "rcl"))*/ //fprintf(stderr, "(pre) build thread on %d\n", core ? core->core_id : -1);
    
 //   fprintf(stderr, "build thread on %d\n", core ? core->core_id : -1);
    //^ Often crashes if here
...
//		printf("****   create thread on core %d\n", core->core_id);
	...
	//fprintf(stderr, "build thread on %d\n", core ? core->core_id : -1);
	// ^shown for S2 when hangs 0 0 0 4
	...
	r->core = core;
	//fprintf(stderr, "build thread on %d\n", core ? core->core_id : -1);
    // ^once shown, once not shown...
	//fprintf(stderr, "build thread on\n");
	// same, once showne once not
    r->arg  = arg;
	r->server_type = server_type;

	//fprintf(stderr, "build routine: %p\n", start_routine);

    //if (core->core_id == 4) fprintf(stderr, "building thread 4");

//	fprintf(stderr, "build thread on %d\n", core ? core->core_id : -1);
	// ^not shown for S2 when hangs : 0 0 0
