...
//    int volatile                    init_done;
//	pthread_cond_t                  cond_state2;             /* condition to wait on state transition */
...
static __attribute__((always_inline)) inline void setprio(pthread_t thread_id, unsigned int prio) {
	...
	int e;
    if((e = pthread_setschedprio(thread_id, prio)))
		fatal("unable to set priority: %s for %d", strerror(e), self.id);
}
...
static struct mini_thread* allocate_mini_thread(struct server* server) {
	...
	//rclprintf(server, "2 CREATE context %p with stack at %p and size %d", res, res->stack, STACK_SIZE);
	makecontext(&res->context, (void(*)())servicing_loop, 0);

	//rclprintf(server, "3 CREATE context %p with stack at %p and size %d", res, res->stack, STACK_SIZE);
	fqueue_enqueue(&server->mini_thread_all, &res->ll_all);

	//rclprintf(server, "4 CREATE context %p with stack at %p and size %d", res, res->stack, STACK_SIZE);
	return res;
}
...
static void servicing_loop() {
    ...
    //if (printed == 0) {
	//    rclprintf(server, "::: start servicing loop %p 2", me->mini_thread);
	//    printed = 1;
    //} 
    // ^Shown when hangs

//	    rclprintf(server, "::: start servicing loop %p 2b", me->mini_thread);
...
//    if (printed == 0) {
//	    rclprintf(server, "::: start servicing loop %p 3", me->mini_thread);
//	    printed = 1;
//    }
    // ^ Shown...
...
//    if (printed == 0) {
//	    rclprintf(server, "::: start servicing loop %p 4", me->mini_thread);
//	    printed = 1;
//    }
// ^ Shown...
	} while(server->state >= SERVER_STARTING);

	//rclprintf(server, "::: start servicing loop %p end", me->mini_thread);
    // ^ Not shown
#ifdef MMM
...
static void* servicing_thread(void* arg) {
...
#endif
	//rclprintf(server, "servicing thread %d, 2", self.id);
	liblock_on_server_thread_start("rcl", self.id);
	//rclprintf(server, "servicing thread %d, 3", self.id);
	getcontext(&me->initial_context);
	//rclprintf(server, "servicing thread %d, 4", self.id);
	if(server->state == SERVER_UP) {
	    //rclprintf(server, "servicing thread %d, 4b", self.id);
    	setcontext(&me->mini_thread->context);
    }

	//rclprintf(server, "servicing thread %d, 5", self.id);
#ifdef EEE
	if (g_monitored_event_id != 0) {
		if(PAPI_stop(event_set, values) != PAPI_OK)
			warning("servicing-loop::PAPI_stop");
	...
	//rclprintf(server, "servicing thread %d, 6", self.id);
	}

	__sync_fetch_and_sub(&server->nb_ready_and_servicing, 1);

	//rclprintf(server, "::: quitting servicing-loop %p", pthread_self());
...
static void* manager_thread(void* arg) {
	//fprintf(stderr, "pre-pre-start:manager\n");
	...
	int nb_not_alive = 0;
#endif
	//fprintf(stderr, "pre-start:manager\n");
   ...
   // rclprintf(server, "ensure 1");
    ...
    //rclprintf(server, "start: manager started");
...
static void launch_server(struct server* server, void (*callback)()) {
	...
	while(server->state != SERVER_DOWN && server->state != SERVER_UP) {
		//rclprintf(server, "launch_server::waiting the server (pre)");
		pthread_cond_wait(&server->cond_state, &server->lock_state);
		//rclprintf(server, "launch_server::waiting the server done (pre)");
	}
	...
	while(server->state == SERVER_STARTING) {
		...
		pthread_cond_wait(&server->cond_state, &server->lock_state);
		//while (!server->init_done) pthread_yield();
...
static void do_liblock_run(rcl)(void (*callback)()) {
	...
	for(i=0; i<topology->nb_cores; i++) {
		...
		if(topology->cores[i].server_type && !strcmp(topology->cores[i].server_type, "rcl")) {
			...
			//fprintf(stderr, "++++ launching: %d from %d\n", i, sched_getcpu());
...
//if (self.id == 3) fprintf(stderr, "pre %d\n", self.id);
	for(i=0; i<topology->nb_cores; i++) {
		servers[i]->requests[self.id].pending = 0;
	}
//if (self.id == 3) fprintf(stderr, "post %d\n", self.id);
...
static void do_liblock_init_library(rcl)() {
	...
	for(i=0; i<topology->nb_cores; i++) {
		...
		//servers[cid]->requests[self.id].pending = 0;
	...
	//	pthread_cond_init(&servers[cid]->cond_state2, 0);
	}
...
// Doesn't work - kills the process
/*
    for (i = 0; i < MAX_NUM_IDS; i++)
        if (selves[i])
            pthread_kill(selves[i], 9);
*/  

    for (i = 0; i < MAX_NUM_IDS; i++)
    {
        if (strlen(log_buffer[i]))
        {
            fprintf(stderr, "ID %d:\n", i);
