...
/** +EDIT */
	...
	if ((ret = __mutex_alloc(
		...
		return (ret);
#else
	//bench_liblock = liblock_lookup(BENCH_LOCKERS_LOCK_TYPE);
	//bench_liblock->init_library(BENCH_LOCKERS_LOCK_CORE);
    //region->mtx_lockers = bench_liblock->create_lock(bench_liblock, BENCH_LOCKERS_LOCK_CORE, 0);
    //bench_liblock->run(0); // Can be called several times (once per lock), taken care of by the lib
    
	if ((ret = __mutex_alloc(
	    env, MTX_LOCK_REGION, 0, &region->mtx_lockers)) != 0)
		return (ret);
    
	liblock_reserve_core_for(BENCH_LOCKERS_LOCK_CORE, BENCH_LOCKERS_LOCK_TYPE);    
    liblock_lock_init(BENCH_LOCKERS_LOCK_TYPE, BENCH_LOCKERS_LOCK_CORE,
    				  &region->liblock_lockers_lock, 0);
#endif
/** -EDIT */
