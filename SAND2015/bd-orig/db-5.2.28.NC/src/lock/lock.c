...
/** +EDIT */
volatile struct __lock_get_api_cs_args_s {
	ENV *env;
	u_int32_t locker;
	DB_LOCKER *sh_locker;
	int ret;
...
void * __lock_get_api_cs(struct cs_args_s *args)
{
	args->ret = __lock_getlocker_int(args->env->lk_handle, args->locker, 0, &args->sh_locker);
}
/** -EDIT */
static int
__lock_get_api(env, locker, flags, obj, lock_mode, lock)
...
#else
	volatile struct __lock_get_api_cs_args_s args __attribute__ ((aligned (8)));
