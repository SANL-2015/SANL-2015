...
volatile struct __dd_build_cs_args_s {
	// Variables used in cs       Also used :      written before (X = parameter)   read after
//	u_int32_t count;              //
	DB_LOCKREGION *region;        //               x                                x
	ENV *env;                     //               X                                x
	u_int32_t nentries;           //                                                x
	int ret;                      //                                                o
	u_int32_t *bitmap;            //                                                x
	u_int32_t *tmpmap;           //                                                 x
	locker_info *id_array;        //                                                x
	u_int32_t id;                 //                                                x
	int *pri_set;                 //               X
//	DB_LOCKER *lip;               //
	u_int32_t atype;              //               X
	...
	u_int32_t gen;                //               x
	DB_LOCKTAB *lt;               //               x                                x
	struct __db_lock *lp;         //               x
	DB_LOCKER *lockerp;           //               x
//	u_int32_t dd;                 //
//	int is_first;                 //
	db_timespec now;              //               x
	int *rejectp;                 //               X
	db_timespec min_timeout;      //               x                                x
//	u_int32_t *entryp;            //
//	DB_LOCKER *child;             //
//	u_int32_t ndx;                //
//	DB_LOCKOBJ *lo;               //
...
void * __dd_build_cs(volatile struct cs_args_s *args)
{
	u_int32_t count;
	int ret;
	DB_LOCKER *lip;
...
//		LOCK_LOCKERS(args->env, args->region);
	...
	printf("Inside critical section : lock_deadlock, CS1. Thread id : %x\n", pthread_self());
#endif
	count = args->region->nlockers;
	if (count == 0) {
		return (void *)(u_int64_t)0;
		/*
	...
	if (FLD_ISSET(args->env->dbenv->verbose, DB_VERB_DEADLOCK))
		__db_msg(args->env, DB_STR_A("2051", "%lu lockers",
			"%lu"), (u_long)count);
	args->nentries = (u_int32_t)DB_ALIGN(count, 32) / 32;
	
	/* Allocate enough space for a count by count bitmap matrix. */
	if ((args->ret = __os_calloc(args->env, (size_t)count,
		sizeof(u_int32_t) * args->nentries, &args->bitmap)) != 0) {
		return (void *)(u_int64_t)1;
		/*
		UNLOCK_LOCKERS(env, region);
		LOCK_SYSTEM_UNLOCK(lt, region);
		return (ret);
	...
	if ((args->ret = __os_calloc(args->env,
		sizeof(u_int32_t), args->nentries, &args->tmpmap)) != 0) {
		return (void *)(u_int64_t)2;
		/*
		UNLOCK_LOCKERS(env, region);
		LOCK_SYSTEM_UNLOCK(lt, region);
		__os_free(env, bitmap);
	...
	if ((args->ret = __os_calloc(args->env,
		(size_t)count, sizeof(locker_info), &args->id_array)) != 0) {
		return (void *)(u_int64_t)3;
		/*
		UNLOCK_LOCKERS(env, region);
		LOCK_SYSTEM_UNLOCK(lt, region);
		__os_free(env, bitmap);
	...
	args->id = 0;
	*args->pri_set = 0;
	SH_TAILQ_FOREACH(lip, &args->region->lockers, ulinks, __db_locker) {
		if (lip->master_locker == INVALID_ROFF) {
			DB_ASSERT(args->env, args->id < count);
			lip->dd_id = args->id++;
			args->id_array[lip->dd_id].id = lip->id;
			args->id_array[lip->dd_id].priority = lip->priority;
			if (lip->dd_id > 0 &&
				args->id_array[lip->dd_id-1].priority != lip->priority)
				*args->pri_set = 1;

			switch (args->atype) {
			case DB_LOCK_MINLOCKS:
			case DB_LOCK_MAXLOCKS:
				args->id_array[lip->dd_id].count = lip->nlocks;
				break;
			case DB_LOCK_MINWRITE:
			case DB_LOCK_MAXWRITE:
				args->id_array[lip->dd_id].count = lip->nwrites;
				break;
			default:
				break;
			}
...
again:		memset(args->bitmap, 0, count * sizeof(u_int32_t) * args->nentries);
	}

	LOCK_DD(args->env, args->region);
	args->op = SH_TAILQ_FIRST(&args->region->dd_objs, __db_lockobj);

	for (; args->op != NULL; args->op = args->np) {
		args->indx = args->op->indx;
		args->gen = args->op->generation;
		UNLOCK_DD(args->env, args->region);

		OBJECT_LOCK_NDX(args->lt, args->region, args->indx);
		if (args->gen != args->op->generation) {
			OBJECT_UNLOCK(args->lt, args->region, args->indx);
			goto again;
		}
		...
		CLEAR_MAP(args->tmpmap, args->nentries);
		SH_TAILQ_FOREACH(args->lp, &args->op->holders, links, __db_lock) {
			args->lockerp = (DB_LOCKER *)R_ADDR(&args->lt->reginfo, args->lp->holder);

			if (args->lockerp->dd_id == DD_INVALID_ID) {
				/*
				 * If the locker was not here when we started,
				 * then it was not deadlocked at that time.
				 */
				if (args->lockerp->master_locker == INVALID_ROFF)
					continue;
				dd = ((DB_LOCKER *)R_ADDR(&args->lt->reginfo,
					args->lockerp->master_locker))->dd_id;
				if (dd == DD_INVALID_ID)
					continue;
				args->lockerp->dd_id = dd;
				switch (args->atype) {
				case DB_LOCK_MINLOCKS:
				case DB_LOCK_MAXLOCKS:
					args->id_array[dd].count += args->lockerp->nlocks;
					break;
				case DB_LOCK_MINWRITE:
				case DB_LOCK_MAXWRITE:
					args->id_array[dd].count += args->lockerp->nwrites;
					break;
				default:
					break;
				}
			} else
				dd = args->lockerp->dd_id;
			args->id_array[dd].valid = 1;
			/*
			 * If the holder has already been aborted, then
			 * we should ignore it for now.
			 */
			if (args->lp->status == DB_LSTAT_HELD)
				SET_MAP(args->tmpmap, dd);
		}
		 ...
		 */
		for (is_first = 1,
			args->lp = SH_TAILQ_FIRST(&args->op->waiters, __db_lock);
			args->lp != NULL;
			is_first = 0,
			args->lp = SH_TAILQ_NEXT(args->lp, links, __db_lock)) {

			args->lockerp = (DB_LOCKER *)R_ADDR(&args->lt->reginfo, args->lp->holder);
			if (args->lp->status == DB_LSTAT_WAITING) {
				if (__clock_expired(args->env,
					&args->now, &args->lockerp->lk_expire)) {
					args->lp->status = DB_LSTAT_EXPIRED;
					MUTEX_UNLOCK(args->env, args->lp->mtx_lock);
					if (args->rejectp != NULL)
						++*args->rejectp;
					continue;
				}
				if (timespecisset(&args->lockerp->lk_expire) &&
					(!timespecisset(&args->min_timeout) ||
					timespeccmp(
					&args->min_timeout, &args->lockerp->lk_expire, >)))
					args->min_timeout = args->lockerp->lk_expire;
			}
			...
			entryp = args->bitmap + (args->nentries * dd);
			OR_MAP(entryp, args->tmpmap, args->nentries);
			/*
			...
			if (is_first) {
				if (ISSET_MAP(entryp, dd))
					args->id_array[dd].self_wait = 1;
				CLR_MAP(entryp, dd);
			}
		}
		LOCK_DD(args->env, args->region);
		args->np = SH_TAILQ_NEXT(args->op, dd_links, __db_lockobj);
		OBJECT_UNLOCK(args->lt, args->region, args->indx);
	}
	UNLOCK_DD(args->env, args->region);
	 ...
	 */
	for (args->id = 0; args->id < count; args->id++) {

		if (!args->id_array[args->id].valid)
			continue;

		if ((args->ret = __lock_getlocker_int(args->lt,
			args->id_array[args->id].id, 0, &args->lockerp)) != 0 || args->lockerp == NULL)
			continue;
		 ...
		 */
		child = SH_LIST_FIRST(&args->lockerp->child_locker, __db_locker);
		if (child != NULL) {
			do {
c_retry:			args->lp = SH_LIST_FIRST(&child->heldby, __db_lock);
				if (SH_LIST_EMPTY(&child->heldby) || args->lp == NULL)
					goto c_next;

				if (F_ISSET(child, DB_LOCKER_INABORT))
					args->id_array[args->id].in_abort = 1;
				ndx = args->lp->indx;
				OBJECT_LOCK_NDX(args->lt, args->region, ndx);
				if (args->lp != SH_LIST_FIRST(
					&child->heldby, __db_lock) ||
					ndx != args->lp->indx) {
					OBJECT_UNLOCK(args->lt, args->region, ndx);
					goto c_retry;
				}

				if (args->lp != NULL &&
					args->lp->status == DB_LSTAT_WAITING) {
					args->id_array[args->id].last_locker_id = child->id;
					goto get_lock;
				} else {
					OBJECT_UNLOCK(args->lt, args->region, ndx);
				}
...
l_retry:	args->lp = SH_LIST_FIRST(&args->lockerp->heldby, __db_lock);
		if (!SH_LIST_EMPTY(&args->lockerp->heldby) && args->lp != NULL) {
			ndx = args->lp->indx;
			OBJECT_LOCK_NDX(args->lt, args->region, ndx);
			if (args->lp != SH_LIST_FIRST(&args->lockerp->heldby, __db_lock) ||
				args->lp->indx != ndx) {
				OBJECT_UNLOCK(args->lt, args->region, ndx);
				goto l_retry;
			}
			args->id_array[args->id].last_locker_id = args->lockerp->id;
get_lock:		args->id_array[args->id].last_lock = R_OFFSET(&args->lt->reginfo, args->lp);
			args->id_array[args->id].last_obj = args->lp->obj;
			lo = SH_OFF_TO_PTR(args->lp, args->lp->obj, DB_LOCKOBJ);
			args->id_array[args->id].last_ndx = lo->indx;
			pptr = SH_DBT_PTR(&lo->lockobj);
			if (lo->lockobj.size >= sizeof(db_pgno_t))
				memcpy(&args->id_array[args->id].pgno,
					pptr, sizeof(db_pgno_t));
			else
				args->id_array[args->id].pgno = 0;
			OBJECT_UNLOCK(args->lt, args->region, ndx);
		}
		if (F_ISSET(args->lockerp, DB_LOCKER_INABORT))
			args->id_array[args->id].in_abort = 1;
	}
...
#else
/* ============================================================================================== */
	volatile struct __dd_build_cs_args_s args __attribute__ ((aligned (8)));

	args.region = region;
	args.env = env;
	args.pri_set = pri_set;
	args.atype = atype;
	args.op = op;
	args.np = np;
	args.indx = indx;
	...
	args.lt = lt;
	args.lp = lp;
	args.lockerp = lockerp;
	args.now = now;
	args.rejectp = rejectp;
	args.min_timeout = min_timeout;
	
	void *cs_result = liblock_exec(&region->liblock_lockers_lock,
								   (void *(*)(void *))&__dd_build_cs, (void *)&args);
...
/** +EDIT */
volatile struct __dd_abort_cs_args_s {
	// Variables used in cs       Also used :      written before (X = parameter)   read after
	int ret;                      //               x                                x
	DB_LOCKTAB *lt;               //               x                                x
	locker_info *info;            //               X
...
void * __dd_abort_cs(volatile struct cs_args_s *args)
{
	DB_LOCKER *lockerp;
	struct __db_lock *lockp;
	DB_LOCKOBJ *sh_obj;
...
//		LOCK_LOCKERS(args->env, args->region);
#ifdef BENCHMARK_DEBUG
	printf("Inside critical section : lock_deadlock, CS2. Thread id : %x\n", pthread_self());
#endif
	 ...
	 */
	if ((args->ret = __lock_getlocker_int(args->lt,
		args->info->last_locker_id, 0, &lockerp)) != 0)
		goto err;
	if (lockerp == NULL || F_ISSET(lockerp, DB_LOCKER_INABORT)) {
		*args->statusp = DB_ALREADY_ABORTED;
		goto err;
	}
	...
	OBJECT_LOCK_NDX(args->lt, args->region, args->info->last_ndx);
	if ((lockp = SH_LIST_FIRST(&lockerp->heldby, __db_lock)) == NULL) {
		*args->statusp = DB_ALREADY_ABORTED;
		goto done;
	}
	if (R_OFFSET(&args->lt->reginfo, lockp) != args->info->last_lock ||
		lockp->holder != R_OFFSET(&args->lt->reginfo, lockerp) ||
	...
	STAT_INC_VERB(args->env, args->lock, args->deadlock,
		args->region->stat.st_ndeadlocks, lockerp->id, &sh_obj->lockobj);
	...
	if (SH_TAILQ_FIRST(&sh_obj->waiters, __db_lock) == NULL) {
		LOCK_DD(args->env, args->region);
		SH_TAILQ_REMOVE(&args->region->dd_objs,
			sh_obj, dd_links, __db_lockobj);
		UNLOCK_DD(args->env, args->region);
	} else
		args->ret = __lock_promote(args->lt, sh_obj, NULL, 0);
	MUTEX_UNLOCK(args->env, lockp->mtx_lock);

done:	OBJECT_UNLOCK(args->lt, args->region, args->info->last_ndx);
err:	;
//		UNLOCK_LOCKERS(args->env, args->region);
}
/** -EDIT */
...
#else
/* ============================================================================================== */
	volatile struct __dd_abort_cs_args_s args __attribute__ ((aligned (8)));
	...
	liblock_exec(&region->liblock_lockers_lock, (void *(*)(void *))&__dd_abort_cs, (void *)&args);
